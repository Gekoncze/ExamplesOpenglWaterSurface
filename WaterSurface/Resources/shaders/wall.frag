...
vec3 blend(vec4 a, vec3 b)
{
    return a.rgb * a.a + b.rgb * (1.0 - a.a);
}

float computeCausticsTransparency()
{
	float size = 0.15;
	float speed = 2.15;
	float p1 = 0.4;
	...
	return clamp((-abs(intensity) + p1) * p2, 0.0, 1.0);
}

void main()
{
	vec3 wall_color;
    if(frag_uv.x < 0.01 || frag_uv.y < 0.01 || frag_uv.x > 0.99 || frag_uv.y > 0.99){
		wall_color = vec3(0.1, 0.1, 0.45);
    } else {
		wall_color = vec3(0.85, 0.85, 0.85);
	}

	if(frag_y < 0.0 && !gl_FrontFacing){
		vec4 caustics_color = vec4(1.0, 1.0, 1.0, computeCausticsTransparency());
		color = vec4(blend(caustics_color, wall_color), 1.0);
	} else {
